...
class CreateSurvey(Page):
    ...
    # Login
    MENU = (By.CLASS_NAME, "hamburger-menu")
    SIGN_IN_LINK_TEXT = (By.LINK_TEXT, "Sign In")
    ...
    def open_page_sm_home_page(self):
        self.open_web_page(URL)

    def click_on_harburger_menu(self):
        self.click(self.MENU)


    def click_on_sign_link(self):
        if self.scroll_to_element_visible(self.SIGN_IN_LINK_TEXT):
            self.click(self.SIGN_IN_LINK_TEXT)
    ...
    def click_on_sign_button(self):
        cli = self.find_element(self.SIGN_IN_BUTTON).is_enabled()
        print "is clickable {} ".format(cli)
        if self.scroll_to_element_visible(self.SIGN_IN_BUTTON):
            sleep(2)
    ...
    def fill_login_details(self, username=None, password=None):
        ...
        if username:
            ...
            print "Filled username {}".format(username)
        if password:
            self.fill_text_box(self.PASSWORD, password)
            print "Filled password {}".format(password)
